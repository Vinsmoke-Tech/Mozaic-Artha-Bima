...
class APIController extends Controller
{
    ...
    public function updatePrinterAddress(Request $request){
        ...
        if($preferencecompany->save()){
            return response([
                'message' => 'Ganti Nama Printer Kasir Berhasil'
            ],201);
        }else{
            return response([
                'message' => 'Ganti Nama Printer Kasir Tidak Berhasil'
            ],401);
        }
    }
    ...
    public function getExpenditureAccount(Request $request){
        ...
        ->where('account_id', 7)
        ->orWhere('account_id', 9)
        ->orWhere('account_id', 10)
        ->orWhere('account_id', 11)
        ->orWhere('account_id', 12)
        ->orWhere('account_id', 13)
        ->orWhere('account_id', 14)
        ->orWhere('account_id', 15)
        ->get();
