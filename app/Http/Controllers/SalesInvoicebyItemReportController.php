...
class SalesInvoicebyItemReportController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = SalesInvoiceItem::select('item_category_id','item_id',DB::raw('sum(`quantity`) as quantity'))
        ->whereHas('invoice', function (Builder $query) use($start_date,$end_date,$company_id) {
           $query->where('sales_invoice_date','>=',$start_date);
           $query->where('sales_invoice_date','<=',$end_date);
           $query->where('company_id', $company_id);
           $query->where('data_state',0);
        })
        ->groupBy('item_id')
        ->get();
